...
function Teams() {
  ...
  return (
    <section><div className="page-heading"><div><div className="eyebrow">Find your people</div><h1>Teams</h1><p>Progress is better together.</p></div></div>
      ...
      <div className="data-grid">{data.map((team, index) => <article className="data-card" key={team._id || team.id || index}><div className="eyebrow">Team</div><h2>{team.name || 'Unnamed team'}</h2><p>{team.description || 'Ready for a new challenge.'}</p><p>{team.memberIds?.length || 0} members</p></article>)}</div>
